...
int capacity = 20;
int curr_person[20][5];
int MAX_DIST = 1000000000;
int preserve_th = 8;
int horizontal = 1;
float count_position = 0.5;
float boundary_frac = 0.02;
int dist_th = 150;
int up = 0;
int down = 0;
...
// ====================================================================
    ...
    */
    ...
    int updown_counter(int up_check, int down_check, int prev_position_x, int prev_position_y, int position_x, int position_y, int position, int delta) {
        if ((prev_position_y >= position + delta) && (position_y < position - delta)) {
            up += 1;
            return 0;
        }
        else if ((prev_position_y >= position + delta) && (position_y < position + delta)) {
            return 1;
        }
        else if ((prev_position_y >= position - delta) && (position_y < position - delta)) {
            if (up_check == 1) {
        ...
        else if ((prev_position_y < position + delta) && (prev_position_y >= position - delta) && (position_y < position + delta) && (position_y >= position - delta)) {
            return 2;
        }
        return 0;
    }

    void person_tracking(int(*person)[2], int num_person, int(*prev_person)[5], int num_prev_person, int position, int delta) {
        printf("Count Boundary: %d - %d\n\n", position - delta, position + delta);

        printf("num prev person: %d\n", num_prev_person);
        for (int i = 0; i < num_prev_person; i++) {
            printf("   %d/ %d   (%d  %d  %d)\n", prev_person[i][0], prev_person[i][1], prev_person[i][2], prev_person[i][3], prev_person[i][4]);
        }
        printf("\n");
        printf("num person: %d\n", num_person);
        ...
        int reset;
        if ((num_prev_person == 0) && (num_person == 0)) {
        }
        else if (num_prev_person == 0) {
            for (int i = 0; i < num_person; i++) {
                prev_person[i][0] = person[i][0];
                prev_person[i][1] = person[i][1];
                prev_person[i][2] = 1;
                prev_person[i][3] = 0;
                prev_person[i][4] = 0;
            }
        }
        else if (num_person == 0) {
            // �� �� �־��µ� �ƿ� ���� ��
            // printf("step3\n");
            int num_delete = 0;
            for (int i = 0; i < num_prev_person; i++) {
                if (prev_person[i - num_delete][2] == preserve_th) {
                    for (int j = i; j < num_prev_person; j++) {
                        prev_person[j][0] = prev_person[j + 1][0];
                        prev_person[j][1] = prev_person[j + 1][1];
                        prev_person[j][2] = prev_person[j + 1][2];
                        prev_person[j][3] = prev_person[j + 1][3];
                        prev_person[j][4] = prev_person[j + 1][4];
                    }
                    num_delete++;
                    prev_person[num_prev_person - num_delete][2] = 0;
                }
                else {
                    prev_person[i - num_delete][2] += 1;
                }
            }
        }
        else {
            ...
            int dist[20][20];
            for (int i = 0; i < num_person; i++) {
                for (int j = 0; j < num_prev_person; j++) {
                    dist[i][j] = (person[i][0] - prev_person[j][0]) * (person[i][0] - prev_person[j][0])
                        + (person[i][1] - prev_person[j][1]) * (person[i][1] - prev_person[j][1]);
                }
            }
            if (num_person == num_prev_person) {
                // printf("Person equal\n");
                // �����̶� ���� ��� �� �� �� �Ȱ��� ��
                // ���� �Ÿ��� ����� pair���� ���ν�Ű�� �� pair�� �����ش�!!
                for (int i = 0; i < num_prev_person; i++) {
                    ...
                    for (int j = 0; j < num_person; j++) {
                        for (int k = 0; k < num_prev_person; k++) {
                            if (min_dist > dist[j][k]) {
                                idx = j;
                                prev_idx = k;
                                min_dist = dist[j][k];
                            }
                    ...
                    reset = updown_counter(prev_person[prev_idx][3], prev_person[prev_idx][4], prev_person[prev_idx][0], prev_person[prev_idx][1], person[idx][0], person[idx][1], position, delta);
                    if (reset == 1) {
                        prev_person[prev_idx][3] += 1;
                    }
                    else if (reset == -1) {
                        prev_person[prev_idx][4] += 1;
                    }
                    else if (reset == 0) {
                        prev_person[prev_idx][3] = 0;
                        prev_person[prev_idx][4] = 0;
                    }
                    prev_person[prev_idx][0] = person[idx][0];
                    prev_person[prev_idx][1] = person[idx][1];
                    prev_person[prev_idx][2] = 1;
                    for (int j = 0; j < num_person; j++) {
                        dist[j][prev_idx] = MAX_DIST;
                    }
                    for (int j = 0; j < num_prev_person; j++) {
                        dist[idx][j] = MAX_DIST;
                    }
            ...
            else if (num_person < num_prev_person) {
                // printf("Person Decrease\n");
                // �������� ���� �پ��� ��
                // ���� ��ų �� �ִ� �ֵ� �� ��Ű�� �����ִ� preserve count�� 1 �÷��ߵ�
                for (int i = 0; i < num_person; i++) {
                    int min_dist = MAX_DIST;
                    int idx = -1;
                    int prev_idx = -1;
                ...
                // ���� �ֵ� �߰����ֱ�
                for (int i = 0; i < num_person; i++) {
                    if (dist[i][0] != MAX_DIST) {
                        prev_person[num_prev_person][0] = person[i][0];
                        prev_person[num_prev_person][1] = person[i][1];
                        prev_person[num_prev_person][2] = 1;
                        num_prev_person += 1;
                    }
                }
            }
        ...
        int idx = 0;
        for (int i = 0; i < num_prev_person; i++) {
            if (prev_person[idx][2] == preserve_th) {
                for (int j = idx; j < num_prev_person - 1; j++) {
                    prev_person[j][0] = prev_person[j + 1][0];
                    prev_person[j][1] = prev_person[j + 1][1];
                    prev_person[j][2] = prev_person[j + 1][2];
                }
                prev_person[num_prev_person - 1][0] = 0;
                prev_person[num_prev_person - 1][1] = 0;
                prev_person[num_prev_person - 1][2] = 0;
            }
            else {
                idx += 1;
            }
    ...
    extern "C" void draw_detections_cv_v3(mat_cv * mat, detection * dets, int num, float thresh)
    {
        try {
            ...
            for (i = 0; i < num; ++i) {
                ...
                if (class_id == 0) {
                    ...
                    for (j = 0; j < num_person; j++) {
                        x_dist = (person[j][0] - mid_x) * (person[j][0] - mid_x);
                        y_dist = (person[j][1] - mid_y) * (person[j][1] - mid_y);
                        if ((x_dist < dist_th) && (y_dist < dist_th)) {
                            dup = 1;
                            break;
                        }
                    }
                    ...
                    if (dup == 0) {
                        person[num_person][0] = mid_x;
                        person[num_person][1] = mid_y;
                        cv::Point pt1, pt2, mid_pt;
                        pt1.x = left;
                        pt1.y = top;
                        pt2.x = right;
                        pt2.y = bot;
                        ...
                        cv::circle(*show_img, mid_pt, 1, color, width);
                        num_person++;
                    }
                }
            }
            ...
            line_color.val[0] = 0;
            line_color.val[1] = 256;
            line_color.val[2] = 0;
            position = (int)(count_position * show_img->rows);
            delta = (int)(boundary_frac * show_img->rows);

            line1_pt1.x = 0;
            line1_pt1.y = position - delta;
            line1_pt2.x = show_img->cols;
            line1_pt2.y = position - delta;

            line2_pt1.x = 0;
            line2_pt1.y = position + delta;
            line2_pt2.x = show_img->cols;
            line2_pt2.y = position + delta;
            cv::line(*show_img, line1_pt1, line1_pt2, line_color, width);
            cv::line(*show_img, line2_pt1, line2_pt2, line_color, width);
            person_tracking(person, num_person, curr_person, num_curr_person, position, delta);
        }
        catch (...) {
